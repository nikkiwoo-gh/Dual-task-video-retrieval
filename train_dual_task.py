...
from model import Dual_Task, get_we_parameter
import logging
...
def parse_args():
    ...
    # model
    parser.add_argument('--model', type=str, default='dual_task', help='model name. (default: dual_task)')
    ...
    parser.add_argument('--tconcate', type=str, default='full', help='feature concatenation style. (full|reduced) full=level 1+2+3; reduced=level 2+3')
    ...
    parser.add_argument('--vconcate', type=str, default='full', help='feature concatenation style. (full|reduced) full=level 1+2+3; reduced=level 2+3')
    ...
    parser.add_argument('--classification_loss_type', type=str, default='favorBCEloss',
                        help='loss function for the classification loss')
    parser.add_argument('--matching_loss_type', type=str, default='Tripletloss',
                        help='loss function for the matching loss')
    ...
    parser.add_argument('--cv_name', default='ACMMM2020', type=str, help='')
...
def main():
    ...
    # checkpoint path
    model_info = '%s_concate_%s_dp_%.1f_measure_%s_lambda_%.1f' %  (opt.model, opt.concate, opt.dropout, opt.measure,opt.multiclass_loss_lamda)
    ...
    concept_file = os.path.join(rootpath, opt.trainCollection, 'TextData', 'concept', 'concept_list_gt5.pkl')
    concept_list = pickle.load(open(concept_file, 'rb'))
    concept2vec = get_text_encoder('bow')(concept_list,istimes=0)
    opt.concept_list_size = len(concept_list)


    # set data loader
    video2frames = {x: read_dict(os.path.join(rootpath, collections[x], 'FeatureData', 'video2frames.txt'))
                    for x in collections }
    data_loaders = data.get_dualtask_data_loaders(
        caption_files, visual_feats, rnn_vocab, bow2vec,concept2vec, opt.batch_size, opt.workers, opt.n_caption, video2frames=video2frames)
       

    # Construct the model
    opt.decoder_mapping_layers = [opt.visual_mapping_layers[-1], opt.concept_list_size]
    model = Dual_Task(opt)
    opt.we_parameter = None
    ...
    for epoch in range(opt.num_epochs):
        ...
        matching_currscore, classification_cur_recall   = evaluation.dual_task_eval(opt, data_loaders['val'], model,concept2vec,opt.measure)

        classification_currscore = classification_cur_recall
        ...
        best_recall = max(best_recall, classification_cur_recall)
    ...
    with open(os.path.join(opt.logger_name, 'val_metric.txt'), 'w') as fout:
        fout.write('best performance on validation: matching' + str(best_matching_currscore) +
                   ', classification' + str(best_classification_currscore))
...
def train_dual_task(opt, train_loader, model, epoch):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        b_size, loss, loss_matching, loss_classification = model.train_emb(*train_data)

        progbar.add(b_size, values=[('loss', loss), ('loss_matching', loss_matching),
