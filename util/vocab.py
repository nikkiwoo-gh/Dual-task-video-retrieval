...
class Concept_phase(object):
    ...
    def add_phase(self, phase):
        if phase not in self.phase2idx:
            self.phase2idx[phase] = self.idx
            self.idx2phase[self.idx] = phase
            self.idx += 1

    def __call__(self, phase):
        if phase not in self.phase2idx:
            return self.phase2idx['<unk>']
        return self.phase2idx[phase]

    def __len__(self):
        return len(self.phase2idx)
