...
class BigFile:
    ...
    def __init__(self, datadir,prefix=None):
        if prefix is None:
            ...
            self.nr_of_images, self.ndims = map(int, open(os.path.join(datadir, 'shape.txt.'+prefix)).readline().split())
            id_file = os.path.join(datadir, "id.txt."+prefix)
        try:
        ...
        if prefix is None:
            ...
            self.binary_file = os.path.join(datadir, "feature.bin."+prefix)
